...
export default function App() {
  ...
  const MAX_FILE_SIZE = 100 * 1024 * 1024; // 100MB (AssemblyAI limit)
  ...
  async function transcribe() {
    ...
    try {
      ...
      setLog(prev => [...prev, `File uploaded successfully in ${requestTime}s`])
      setLog(prev => [...prev, `Transcription started (ID: ${data.id}). Polling for results...`])
      
      // Start polling for results
      const interval = setInterval(async () => {
        try {
          const pollRes = await fetch(`/api/transcript/${data.id}`)
          const pollData = await pollRes.json()
          
          if (!pollRes.ok) {
            throw new Error(pollData.error || `HTTP ${pollRes.status}`, {
              cause: { details: pollData.details || "Unknown error during polling" }
            })
          }
          ...
          if (pollData.status === 'completed') {
            ...
            const chunks = pollData.words?.map(w => ({
            ...
            setSegs(chunks)
            setStatus('done')
            setLog(prev => [...prev, `Transcription completed successfully! (${pollData.text?.length || 0} characters)`])
          } else if (pollData.status === 'error') {
            clearInterval(interval)
            setPollingInterval(null)
            
            throw new Error("Transcription failed", {
              cause: { details: pollData.error || "Unknown error from AssemblyAI" }
            })
          } else {
            setLog(prev => [...prev, `Status: ${pollData.status} (${pollData.progress || 0}%)`])
          }
        } catch (err) {
          clearInterval(interval)
          setPollingInterval(null)
          ...
          setStatus('error')
          setErrorDetails(err.cause?.details || "No additional details available")
          setLog(prev => [...prev, `Polling error: ${err.message}`])
        }
    ...
    <div className="container">
      <div className="app-container">
        ...
        <div className="footer">
          <p>Powered by AssemblyAI Speech-to-Text API</p>
        </div>
      </div>
    </div>
  )
